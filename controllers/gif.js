...
exports.getGifAndComments = (request, response) => {
  ...
  pool.query(text, [gifid], (error, res) => {
    ...
    if (!res.rows[0] || !res || res === undefined) {
      return response.status(404).send('Sorry! No such image ID')
    }
    const { itemid, createdon, title, imageurl, firstname, lastname } = res.rows[0]
